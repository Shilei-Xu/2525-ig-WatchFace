  ...
  allParticles.forEach(p => {
    ...
    for (let i = 0; i < 3; i++) {
      const subParticle = new TrailParticle(
        p.x, p.y, 
        p.r * 0.6, 
        p.type === 'hour' ? color(200) : color(100),
        p.type
      );
...
class TrailParticle {
  ...
  getDefaultColor() {
    if (this.type === 'hour') return color(200);     // 浅灰
    if (this.type === 'minute') return color(100);   // 深灰
    return color(57, 255, 20); // 统一青蓝色
  }
...
function drawClockArms() {
  ...
  if (minuteTrail.length < 30) {
    ...
    minuteTrail.push(new TrailParticle(width/2 + minX, height/2 + minY, 16, color(100), 'minute'));
  }
  
  if (hourTrail.length < 20) {
    let hourX = cos(hourAngle) * hourLength;
    let hourY = sin(hourAngle) * hourLength;
    hourTrail.push(new TrailParticle(width/2 + hourX, height/2 + hourY, 20, color(200), 'hour'));
  }
  ...
  for (let i = 0; i < 10; i++) {
    if (secondBallTrails[i].length < 25) {
      ...
      secondBallTrails[i].push(
        new TrailParticle(width/2 + x, height/2 + y, r, color(0, 191, 255), 'secondBall')
      );
    }
  }
