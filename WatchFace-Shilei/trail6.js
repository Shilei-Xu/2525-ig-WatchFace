...
function mousePressed() {
  if (!physicsEnabled) {
    ...
    currentActivationIndex = 0; // 重置索引
    
    // 逐个激活粒子
    activationInterval = setInterval(() => {
      if (currentActivationIndex < allParticles.length) {
        const p = allParticles[currentActivationIndex];
        p.enablePhysics();
        
        // 为当前粒子添加爆炸力
        const angle = random(TWO_PI);
        const force = random(3, 6);
        if (p.body) {
          Body.setVelocity(p.body, {
            x: cos(angle) * force * random(0.9, 1.1),
            y: sin(angle) * force * random(0.9, 1.1)
          });
